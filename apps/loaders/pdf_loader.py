...
class PDFLoader:
    ...
    def __init__(
        ...
        logger.info(
            f"Initializing PDF Loader with directory - {directory} - "
    ...
    def load_documents(self):
        pdf_files = list(self.directory.rglob("*.pdf"))

        logger.debug(
            f"Loading all pdf files from {self.directory} - "
            f"Found {len(pdf_files)} to be loaded"
        )
